...
async function อ่านใบลาจากฐานข้อมูล() {
  try {
    ...
    // ข้อความบนหัวหน้าให้ตรงกับสิ่งที่เห็นจริง
    // ผู้ขอลาเห็นเฉพาะใบตัวเอง จะเขียนว่า "ทั้งหมดในระบบ" ไม่ได้
    if (!เป็นผู้อนุมัติ(ผู้ใช้)) {
      document.querySelector(".subtitle").textContent = "ใบขอลาของคุณ กดที่แถวเพื่อดูรายละเอียด";
    }
...
function แสดงตาราง(รายการ) {
  if (รายการ.length === 0) {
    กล่อง.innerHTML = เป็นผู้อนุมัติ(ผู้ใช้)
      ? "<p>ยังไม่มีใบขอลาในระบบ</p>"
      : '<p>คุณยังไม่มีใบขอลา</p><p class="hint">กดปุ่ม ยื่นใบลาใหม่ ด้านบนเพื่อเริ่มต้น</p>';
    return;
  }
