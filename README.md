...
## 🌐 เว็บออนไลน์ — เปิดใช้งานได้จริง

### 👉 https://gen-lang-client-0091628886.web.app

> 🔑 **ต้องสมัครสมาชิกหรือเข้าสู่ระบบก่อน** จึงจะเห็นข้อมูล — ไม่ได้ล็อกอินจะอ่านอะไรไม่ได้เลย
> เป็นไปตาม Security Rules ที่ตั้งไว้ ไม่ใช่ระบบขัดข้อง · บัญชีที่สมัครใหม่จะได้บทบาท `employee` เสมอ
>
> 🤖 **ปุ่มผู้ช่วย AI จะไม่ปรากฏบนเว็บออนไลน์** เพราะคีย์ OpenRouter ไม่ได้ถูกส่งขึ้นเซิร์ฟเวอร์
> (โค้ดที่รันในเบราว์เซอร์ซ่อนคีย์ไม่ได้) · ใช้งานได้เฉพาะเครื่องที่มีไฟล์ `js/ai-key.js`
> แต่**สรุปที่ AI เคยเขียนไว้แล้วจะแสดงบนเว็บออนไลน์ได้ตามปกติ**

**เอกสารประกอบในโปรเจกต์**

| ไฟล์ | เนื้อหา |
|---|---|
| [`CLAUDE.md`](CLAUDE.md) | โครงสร้างระบบ · ขอบเขตรายสัปดาห์ · กับดักที่เจอมาแล้ว |
| [`ACL.md`](ACL.md) | ตารางสิทธิ์ 3 บทบาท |
| [`firestore.rules`](firestore.rules) | กฎเฝ้าข้อมูลที่ใช้งานอยู่จริง |
| [`leaveeasy-spec.md`](leaveeasy-spec.md) | ข้อกำหนดของระบบ (ผู้สอนกำหนด) |
...
## ✅ ระบบต่อกับ Firestore แล้ว

ทุกหน้าอ่านและเขียนข้อมูลกับ **Cloud Firestore** ของจริง — **ไม่มีหน้าไหนใช้ `js/data.js` แล้ว**
กรอกใบลา → ปิดเบราว์เซอร์ → เปิดใหม่ → ข้อมูลยังอยู่

> `js/data.js` ยังอยู่ในโปรเจกต์เพราะ spec ระบุไว้ในโครงไฟล์ แต่ไม่มีใครเรียกใช้แล้ว
