...
async function แก้ประเภท(id) {
  ...
  try {
    ...
    วาดตาราง();   // วาดใหม่เพื่อเปิดปุ่มคืน
  }
}
...
// ปิดปุ่มทั้งแถวระหว่างเขียนลงฐาน ตามแบบแผนใน CLAUDE.md
// สำเร็จแล้ว วาดตาราง() จะสร้างปุ่มใหม่ให้เอง จึงไม่ต้องเปิดคืนเอง
function ปิดปุ่มในแถว(id, ข้อความ) {
  var ปุ่มแก้ = ที่วางตาราง.querySelector('[data-edit="' + id + '"]');
  var ปุ่มลบ = ที่วางตาราง.querySelector('[data-del="' + id + '"]');
  [ปุ่มแก้, ปุ่มลบ].forEach(function (ปุ่ม) { if (ปุ่ม) ปุ่ม.disabled = true; });
  if (ปุ่มแก้) ปุ่มแก้.textContent = ข้อความ;
}
